...
import (
	...
	"gitlab.eurecom.fr/open-exposure/nef/ue-address/pkg/app"
)
...
func main() {
	log.Default().Printf("================ ue-address api service for open-exposure ================")

	log.Printf("current git branch: %s", gitBranch)
	log.Printf("current git commit: %s", gitCommit)

	app, err := app.InitApplication("ue-address", getConfigPath())
	if err != nil {
