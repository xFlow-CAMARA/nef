...
func NewNorthbound(app appCtx) (*NbiServer, error) {
	...
	ASSessionWithRequiredQoSSubscriptionsAPIService := NewASSessionWithRequiredQoSSubscriptionsAPIService(nbi)
	IndividualSessionWithQoSDocumentAPIController := NewASSessionWithRequiredQoSSubscriptionsAPIController(ASSessionWithRequiredQoSSubscriptionsAPIService)

	IndividualASSessionWithRequiredQoSSubscriptionAPIService := NewIndividualASSessionWithRequiredQoSSubscriptionAPIService(nbi)
	SessionsWithQoSCollectionAPIController := NewIndividualASSessionWithRequiredQoSSubscriptionAPIController(IndividualASSessionWithRequiredQoSSubscriptionAPIService)

	nbi.router = models.NewRouter(IndividualSessionWithQoSDocumentAPIController, SessionsWithQoSCollectionAPIController)
	...
	if len(nbi.Cfg().CapifSvc) > 0 {
		...
		for rName, route := range IndividualSessionWithQoSDocumentAPIController.Routes() {
			nbi.capifCtx.AddEndpoint(route.Pattern, "SUBSCRIBE_NOTIFY", []string{route.Method}, rName)
		}
		for rName, route := range SessionsWithQoSCollectionAPIController.Routes() {
			nbi.capifCtx.AddEndpoint(route.Pattern, "SUBSCRIBE_NOTIFY", []string{route.Method}, rName)
		}
